...
type MDXSource = MDXRemoteSerializeResult<
...
function calculateNavigation(currentPost: Post, posts: Post[]) {
  ...
  const currentIndex = posts.findIndex(
    ({ filePath }) => filePath == currentPost.filePath
  );
  if (currentIndex === -1)
    throw new Error(
      `calculateNavigation: cannot find current post index:\ncurrentPost.filePath: ${JSON.stringify(
        currentPost.filePath
      )}`
    );
  if (currentIndex === 0)
    return {
      previousPost: posts.length > 1 ? posts[currentIndex + 1] : null,
      nextPost: null,
    };
  if (currentIndex === posts.length - 1)
    return {
      previousPost: null,
      nextPost: posts.length > 1 ? posts[currentIndex - 1] : null,
    };
  return {
